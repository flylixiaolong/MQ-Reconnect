 ...
 * 创建MQ连接客户端
 */
function Client(name, url, interval, confirm, logger, socketOptions) {
  ...
  switch (typeof logger) {
    case 'object':
      if(typeof logger.info == 'function') {
        ...
        this.socketOptions = logger;
        this.logger = console;
      }
      break;
  }
  switch (typeof socketOptions) {
    case 'object':
...
Client.prototype.connect = function(immediate){
  ...
  var channel = amqp
    .connect(client.url, client.socketOptions)
    .then(function(conn){
      client.conn = conn;
      // 绑定事件  
