...
Client.prototype.connect = function(immediate){
  ...
  client.conn = amqp
    ...
    .catch(function(err){
      client.state = 'unconnected';
      client.logger.warn('[%s]:%s', client.name, 'connect failure');
      client.reconnect();
...
Client.prototype.consume = function(queue, handle, options, reconnect) {
  ...
  return client.connect()
    ...
    .then(function(){
      return channel.consume(queue, handle, options);
    })
    .then(function(res){
      client.logger.info('[%s]:%O,%s,%O', client.name, args, 'bind consume success', res);
    })
    .catch(function(err){
      client.logger.warn('[%s]:%O,%s', client.name, args, err.message || 'bind consume error');
    });
}
